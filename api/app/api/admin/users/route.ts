...
// Owner-only, same boundary as admin/provision-deos/route.ts — the actual gate is this check,
// not the ProfileMenu link's visibility. Returns the requesting admin's own row (so the
// frontend can flag the owner row as un-deletable/un-editable) or null if the caller isn't the
// owner — callers respond 401/403 themselves so the specific status code stays visible at the
// call site, same as every other route in this codebase.
async function getOwnerActor(req: NextRequest, db: ReturnType<typeof getDb>): Promise<AdminActor | null> {
  ...
  if (!admin?.email || admin.email !== process.env.OWNER_EMAIL) return null;
  return admin;
}
...
export const GET = withErrorHandling("admin/users", async (req: NextRequest) => {
  ...
  return NextResponse.json({
    rows: rows.map((r) => ({ ...r, isOwner: r.email === process.env.OWNER_EMAIL })),
  });
});
...
export const POST = withErrorHandling("admin/users/create", async (req: NextRequest) => {
  ...
  await db.insert(users).values({ role: "admin", email, name, designation: designation || null });
...
export const PATCH = withErrorHandling("admin/users/update", async (req: NextRequest) => {
  ...
  if (body.email !== undefined) {
    const email = typeof body.email === "string" ? body.email.trim() : "";
    if (!email || !EMAIL_RE.test(email)) return NextResponse.json({ error: "A valid email is required" }, { status: 400 });
    if (target.email === process.env.OWNER_EMAIL && email !== target.email) {
...
export const DELETE = withErrorHandling("admin/users/delete", async (req: NextRequest) => {
  ...
  const [target] = await db.select().from(users).where(eq(users.id, targetId)).limit(1);
  if (!target || target.role !== "admin") return NextResponse.json({ error: "Admin user not found" }, { status: 404 });
  if (target.email === process.env.OWNER_EMAIL) return NextResponse.json({ error: "The owner account cannot be deleted" }, { status: 400 });
